...
public class Control {
	...
	loop:			while(true){
					...
					if(proid==0){
						if(list!=null){
							...
							if(s.equals("y")&&i==1){
								//更新会员积分
								updateUser = this.ubiz.updateUser(user,list);
								//存入订单表
	...
	public void showMenu(){
		...
		List<Product> showPro = this.pbiz.showPro();
	...
	//显示购物车列表的方法
		public void showCart(List<Product> list){
			...
			for (Product pro : list) {
				...
				System.out.println(pro.getProductId()+"\t\t"+pro.getProductName()+"\t"+pro.getProductPrice()+"\t\t"+pro.getProductNum()+"\t\t"+sumPrice);
				sum=sum+sumPrice;
...
//查看订单
		public void selectAllOrder(Users u) {
			...
			for (Orders orders : listorder) {
				...
				System.out.println(orders.getOrderNo()+"\t"+orders.getSumPrice()+"\t\t"+orders.getOrderDate()+"\t"+orders.getIsSend()+"\t\t"+orders.getStatus());
			}
			...
			for (Orders orders : Unreceived) {
				...
				System.out.println(orders.getOrderId()+"\t\t"+orders.getOrderNo()+"\t"+orders.getSumPrice()+"\t\t"+orders.getOrderDate()+"\t"+orders.getIsSend()+"\t\t"+orders.getStatus());
			}
...
public void showActiveMenu(Users u) {
	...
	List<Activity> showAct = this.abiz.showAct(u);
