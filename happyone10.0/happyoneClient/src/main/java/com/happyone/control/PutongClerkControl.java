...
import com.happyone.util.ExcelTrueTicket;
...
public class PutongClerkControl {
	...
	public void printlnSonOrder() {
		...
		if(chooseOrderid>(list.size() )){
			this.ui.println("未找到该订单");
			return;
		}
		...
		this.ui.println("商品编号\t商品名称\t商品数量\t商品单价");
		...
		if (list.get(chooseOrderid - 1).getIsSend().equals("否")) {
			...
			//生成实体小票
			ExcelTrueTicket ett=new ExcelTrueTicket(list.get(chooseOrderid - 1), plist,clerk);
	        ett.setExcel();
	        Discuss d=new Discuss(1, list.get(chooseOrderid - 1).getOrderNo(), "好评", "到店就餐自动好评");
