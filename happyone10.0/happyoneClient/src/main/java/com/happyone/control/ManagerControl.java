...
public class ManagerControl {
	...
	public void managerOrders() {
		...
		while (true) {
			...
			case 1:
				list = this.sb.selectOrdersBytype("交易完成");
				if (list.size() != 0) {
					this.ui.println("订单编号\t订单号\t\t\t订单用户id\t订单时间\t订单总价\t是否配送\t订单状态\t评价类型\t评价内容");
					for (int i = 0; i < list.size(); i++) {
						Discuss discuss = this.sb.selectDiscussByno(list.get(i).getOrderNo());
						this.mv.println(list.get(i).toString() + "\t" + discuss.getDiscussType() + "\t"
								+ discuss.getDiscussContent());
					}
			...
			case 6:
				...
				if (prolist.size() > 0) {
					...
					for (int i = 0; i < prolist.size(); i++) {
						this.mv.println(
								(i + 1) + "\t" + prolist.get(i).toString() + "\t" + prolist.get(i).getIsShelves());
					}
				}
			
				int proid = this.ui.getInt("请输入查询的商品编号");
				if(proid>prolist.size()){
					this.ui.println("未找到商品");
					continue;
				}
				...
				for (int i = 0; i < plist.size(); i++) {
					if (plist.get(i).getProductId() == productid) {
						count = plist.get(i).getProductNum() + count;
						psum = (plist.get(i).getProductNum() * plist.get(i).getProductPrice()) + psum;
					}
				}
