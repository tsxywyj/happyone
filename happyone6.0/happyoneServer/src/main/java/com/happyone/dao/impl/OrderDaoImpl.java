...
public class OrderDaoImpl implements OrderDao {
	...
	//添加子订单
		public boolean inserSonOrder(List<Product> list, String orderno) {
		    for (Product product : list) {
			...
			for (Product product : list) {
				...
				try {
					int i = db.update(sql,orderno,productId,productNum);
...
//查看未收货订单
	public List<Orders> selectOrderUnreceived(Users u) {
		...
		int id = u.getUserid();
		this.db=new DBUtil();
		String sql="select * from orders where userid=? and issend='是' and status='配送中' or status='配送完成'";
		try {
