...
public class PutongClerkControl {
	...
	public PutongClerkControl(Clerk clerk) {
	...
	public void selectOrderBytype() {
		List<Orders> list = this.mob.selectOrdersBytype("已付款");
		if (list.size() != 0) {
	...
	public void printlnSonOrder() {
		...
		List<Product> plist = this.mob.selectSonOrderByOrderNo(list.get(chooseOrderid - 1).getOrderNo());
		...
		if (list.get(chooseOrderid - 1).getIsSend().equals("否")) {
			this.mob.updateOrdersByid(list.get(chooseOrderid - 1).getOrderId(), "交易完成");
			this.ui.println("打印完成");
		} else {
			// 生成配送订单
			this.mob.updateOrdersByid(list.get(chooseOrderid - 1).getOrderId(), "等待配送");
	...
	// 安排配送
	public void Send() {
		List<Orders> list = this.mob.selectOrdersBytype("等待配送");
		if (list.size() != 0) {
			this.ui.println("订单编号\t订单号\t订单用户id\t订单时间\t订单总价\t是否配送\t订单状态");
			for (int i = 0; i < list.size(); i++) {
				System.out.println((i + 1) + "\t" + list.get(i).getOrderNo() + "\t" + list.get(i).getUserId() + "\t"
		...
		Clerk sendClerk = this.mcb.selectSendClerkBytype("工作中空闲");
		if (sendClerk == null) {
		...
		if (this.mob.insertSendOrder(orderNo, sendClerk.getClerkId())
				&& this.mcb.updateClerkStatus(sendClerk.getClerkId(), "工作中忙碌")
				&& this.mob.updateOrdersByid(list.get(chooseOrder - 1).getOrderId(), "配送中")) {
	...
	public void selectSendOrder() {
		List<SendOrders> list = this.mob.selectSendOrderBytype("配送中");
		if (list.size() != 0) {
