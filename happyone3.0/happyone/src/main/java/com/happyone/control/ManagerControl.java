...
import com.happyone.biz.impl.ManagerActivityBizImpl;
import com.happyone.biz.impl.ManagerClerkBizImpl;
import com.happyone.biz.impl.ManagerOrdersBizImpl;
import com.happyone.biz.impl.ManagerProductBizImpl;
...
public class ManagerControl {
	...
	private ManagerProductBiz mpb;
	private ManagerActivityBiz mab;
	private ManagerOrdersBiz mob;

	public ManagerControl(Clerk clerk) {
		// TODO Auto-generated constructor stub
		this.ui = new UserInput();
		this.mcb = new ManagerClerkBizImpl();
		this.clerk = clerk;
		this.mv = new ManagerView();
		this.mpb = new ManagerProductBizImpl();
		this.mab = new ManagerActivityBizImpl();
		this.mob = new ManagerOrdersBizImpl();
	...
	public void managerStart() {
		while (true) {
			...
			if (chooseAll == 0) {
				this.mcb.updateClerkStatus(clerk.getClerkId(), "下班");
				return;
			}
		}
	}
	...
	public void managerClerk() {
		while (true) {
			...
			switch (chooseClerk) {
			case 1:
				List<Clerk> listAll = this.mcb.selectClerkAll();
			...
			case 2:
				...
				Clerk clerkBytel = this.mcb.selectClerkBytel(clerkTel);
				if (clerkBytel != null) {
					this.mv.println("职工编号\t职工姓名\t职工手机号\t\t职工密码\t职工类型\t职工状态");
			...
			case 3:
				...
				List<Clerk> listtype = this.mcb.selectClerkBytype(selecttype);
				if (listtype != null) {
					this.mv.println("职工编号\t职工姓名\t职工手机号\t\t职工密码\t职工类型\t职工状态");
			...
			case 4:
				...
				if (this.mcb.insertClerk(1, cname, type, ctel, cpasswork, "下班")) {
			...
			case 5:
				...
				if (this.mcb.updateClerkByid(clerk.getClerkId(), updates, updatei)) {
				...
				break;
			case 6:
				List<Clerk> listAll2 = this.mcb.selectClerkAll();
				this.mv.println("职工编号\t职工姓名\t职工手机号\t\t职工密码\t职工类型\t职工状态");
				...
				if (this.mcb.updateClerkByid(listAll2.get(choosetype - 1).getClerkId(), type2, 4)) {
	...
	public void managerProduct() {
		...
		while (true) {
			...
			case 1:
				list = this.mpb.selectProductAll();
				if (list.size() > 0) {
			...
			case 2:
				...
				list = this.mpb.selectProductBytype(productType);
				if (list.size() > 0) {
					this.mv.println("商品编号\t商品名称\t\t商品类型\t商品价格\t是否上架");
					for (int i = 0; i < list.size(); i++) {
						this.mv.println((i + 1) + "\t" + list.get(i).toString() + "\t" + list.get(i).getIsShelves());
			...
			case 3:
				...
				if (this.mpb.insertProduct(0, productNewName, productNewType, productNewPrice, "是")) {
					this.mv.println("添加成功");
				...
				break;
			case 4:
				list = this.mpb.selectProductByis("否");
				if (list.size() > 0) {
					for (int i = 0; i < list.size(); i++) {
						this.mv.println((i + 1) + "\t" + list.get(i).toString() + "\t" + list.get(i).getIsShelves());
					}
					...
					if (updateWha == 2) {
						...
						if (this.mpb.updateProductByid(list.get((updateId - 1)).getProductId(), updateThing,
								updateWha)) {
					...
					if (this.mpb.updateProductByid(list.get((updateId - 1)).getProductId(), updateThing, updateWha)) {
			...
			case 5:
				...
				if (list.size() > 0) {
					...
					if (this.mpb.upProductByid(list.get((upChoose - 1)).getProductId())) {
				...
				break;
			case 6:
				list = this.mpb.selectProductByis("是");
				if (list.size() > 0) {
					for (int i = 0; i < list.size(); i++) {
						this.mv.println((i + 1) + "\t" + list.get(i).toString() + "\t" + list.get(i).getIsShelves());
					}
					int upChoose = this.ui.getInt("输入下架的商品");
					if (this.mpb.downProductByid(list.get((upChoose - 1)).getProductId())) {
	...
	public void managerActivity() {
		...
		while (true) {
			...
			switch (chooseProduct) {
			case 1:
				list = this.mab.selectActivityAll();
				if (list.size() > 0) {
			...
			case 2:
				...
				List<Product> plist = this.mpb.selectProductBytype(productNewType);
				boolean b = false;
				...
				if (this.mab.insertActivity(activityNewName, activityContent, newProductID, productNewPrice, userTypeId,
						"是")) {
					this.mv.println("添加成功");
				...
				break;
			case 3:
				list = this.mab.selectActivityByis("否");
				if (list.size() > 0) {
					for (int i = 0; i < list.size(); i++) {
						this.mv.println((i + 1) + "\t" + list.get(i).toString() + "\t" + list.get(i).getUserTypeId()
								+ "\t" + list.get(i).getIsshelves());
					...
					String updateThing = this.ui.getString("请输入新的信息");
					if (this.mab.updateActivityByid(list.get((updateId - 1)).getActivityId(), updateThing, updateWha)) {
			...
			case 4:
				...
				if (list.size() > 0) {
					...
					if (this.mab.upActivityByid(list.get((upChoose - 1)).getActivityId())) {
						System.out.println("上架成功");
					} else {
						System.out.println("上架失败");
					}
				...
				break;
			case 5:
				list = this.mab.selectActivityByis("是");
				if (list.size() > 0) {
					for (int i = 0; i < list.size(); i++) {
						this.mv.println((i + 1) + "\t" + list.get(i).toString() + "\t" + list.get(i).getIsshelves());
					}
					int upChoose = this.ui.getInt("输入下架的活动");
					if (this.mab.downActivityByid(list.get((upChoose - 1)).getActivityId())) {
						System.out.println("下架成功");
					} else {
						System.out.println("下架失败");
					}
	...
	public void managerOrders() {
		...
		while (true) {
			...
			case 1:
				...
				break;
			case 2:
				slist = this.mob.selectSendOrderAll();
				if (slist.size() != 0) {
			...
			case 5:
				...
				break;
			case 6:
				List<Product> prolist=this.mpb.selectProductAll();
				if (prolist.size() > 0) {
					this.mv.println("商品编号\t商品名称\t\t商品类型\t商品价格\t是否上架");
				...
				plist=this.mob.selectSonOrderAll();
				int count=0;
				double psum=0;
				for(int i=0;i<plist.size();i++){
