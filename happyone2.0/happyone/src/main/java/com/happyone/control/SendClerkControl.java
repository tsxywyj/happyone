...
public class SendClerkControl {
	...
	public SendClerkControl(Clerk clerk) {
	...
	public void selectNewSend() {
		List<SendOrders> list = this.mob.selectSendOrderByclerkid(clerk.getClerkId(), "配送中");
		if (list.size() != 0) {
	...
	public void sendOK() {
		...
		if (this.mob.updateSendOrder(list.get(chooseSend - 1).getSendOrdersId(), "已送达")
				&& this.mob.updateOrdersByNo(list.get(chooseSend - 1).getOrderNo(), "交易完成")) {
	...
	public void selectAllSendorderByid(){
		List<SendOrders> list = this.mob.selectSendOrderByclerkid(clerk.getClerkId(), "配送中");
		List<SendOrders> list2 = this.mob.selectSendOrderByclerkid(clerk.getClerkId(), "已送达");
