...
public class Control {
	...
	// 用户页面
	public void usershelf(boolean b) {
		while (b) {
			this.v.userLogin();
		...
		if (s.equals("y")) {
			while (true) {
				...
				if (od.getStatus().equals("交易完成")) {
					...
					while (true) {
						String isdis = this.ui.getString("是否评价外卖订单？(y/n)");
						...
						if (isdis.equals("y")) {
							...
							if (discontent.indexOf("草") != -1 || discontent.indexOf("妈") != -1
									|| discontent.indexOf("操") != -1 || discontent.indexOf("逼") != -1
									|| discontent.indexOf("fuck") != -1) {
								this.ui.println("评价不合法");
								;
								continue;
							}
							...
							return;
						} else if (isdis.equals("n")) {
							Discuss d = new Discuss(1, orderNo, "好评", "未评价");
							String insertDiscuss = this.sb.insertDiscuss(d);
							return;
						} else {
							this.ui.println("输入有误");
							continue;
						}
					}
				}
