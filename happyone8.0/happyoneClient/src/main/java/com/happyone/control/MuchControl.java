...
public class MuchControl {
	...
	public void busySend() {
		...
		for (int i = 0; i < clist.size(); i++) {
			...
			arr[i] = many;
			this.ui.println("配送员：" + clist.get(i).getClerkName() + "   配送了" + many + "单");
		}
		int temp = 0;
	...
	public void goodProduct() {
		...
		for (int i = 0; i < plist.size(); i++) {
			...
			this.ui.println(plist.get(i).getProductName() + "   累计销售了" + many + "件");
	...
	public void selectDiscussBytpye(){
		this.ui.println("1.好评");
		this.ui.println("2.中评");
		this.ui.println("3.差评");
		int chooseTpye=this.ui.getInt("请选择评价类型");
		String discussType="";
		switch(chooseTpye){
		case 1:
			discussType="好评";
		...
		case 3:
			...
			break;
		default :
			this.ui.println("输入不合法");
			return;
		}
		List<Orders> olist=this.sb.selectOrdersBytype("交易完成");
		int q=1;
		for (int i = 0; i < olist.size(); i++) {
			Discuss discuss=this.sb.selectDiscussByno(olist.get(i).getOrderNo());
			if(discuss.getDiscussType().equals(discussType)){
			q=2;
			this.ui.println("订单编号\t订单号\t\t\t订单用户id\t订单时间\t订单总价\t是否配送\t订单状态\t评价类型\t评价内容");
			break;
				//this.mv.println(olist.get(i).toString()+"\t"+discuss.getDiscussType()+"\t"+discuss.getDiscussContent());
			}
		}
		if(q==1){
			this.ui.println("没有该评价等级的订单");
			return;
		}
		for (int i = 0; i < olist.size(); i++) {
			Discuss discuss=this.sb.selectDiscussByno(olist.get(i).getOrderNo());
