		...
		while (true) {
			...
			case 4:
				...
				if (list.size() > 0) {
					this.mv.println("活动编号\t活动名称\t\t活动描述\t\t活动商品名称\t活动价格\t会员等级限制\t是否上架");
					for (int i = 0; i < list.size(); i++) {
						this.mv.println((i + 1) + "\t" + list.get(i).toString() + "\t" + list.get(i).getUserTypeId()+ "\t\t" +list.get(i).getIsshelves());
					}
			...
			case 1:
				...
				if (list.size() != 0) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						this.mv.println(list.get(i).toString()+"\t"+discuss.getDiscussType()+"\t"+discuss.getDiscussContent());
					}
